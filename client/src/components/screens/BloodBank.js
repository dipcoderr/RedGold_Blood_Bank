...
function BloodBank() {
    ...
    useEffect(() => {
        ...
        fetch(`${config.API_URL}/api/blood/all`, {
            method: "get",
            headers: {
    ...
    const addBloodGroup = (e) => {
        ...
        fetch(`${config.API_URL}/api/blood/add`, fetchData)
        .then(res => res.json())
        .then(data2 => {
            setLoading(false);
            if(data2.error) {
    ...
    const bookBlood = (bloodId) => {
        ...
        fetch(`${config.API_URL}/api/booking/new`, {
            method: "post",
            headers: {
                "Content-Type": "application/json",
                "Authorization":"Bearer "+JSON.parse(localStorage.getItem("jwt"))
    ...
    const deleteBlood = (bloodId) => {
        ...
        fetch(`${config.API_URL}/api/blood/delete/${bloodId}`, fetchData)
        .then(res => res.json())
        .then(data2 => {
            if(data2.error) {
                toast.error(data2.error)
